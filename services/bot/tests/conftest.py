...
_AIOGRAM_TYPES = [
    ...
    "ChatMemberUpdated", "Poll", "PollOption", "Document", "PhotoSize",
...
class _TransitionSentinel:
    ...
    def __rshift__(self, other): return MagicMock()
    def __lshift__(self, other): return MagicMock()
    def __rrshift__(self, other): return MagicMock()

_mod("aiogram",
     Router=MagicMock, F=MagicMock(), Bot=MagicMock,
     IS_MEMBER=_TransitionSentinel(), IS_NOT_MEMBER=_TransitionSentinel())

def _noop_filter(*a, **kw): return MagicMock()

_mod("aiogram.filters",
     CommandStart=_noop_filter, Command=_noop_filter,
     StateFilter=_noop_filter, ChatMemberUpdatedFilter=_noop_filter,
     IS_MEMBER=_TransitionSentinel(), IS_NOT_MEMBER=_TransitionSentinel())
...
_mod("aiogram.webhook.aiohttp_server",
     SimpleRequestHandler=MagicMock, setup_application=MagicMock)
...
_mod("sqlalchemy",
     ...
     BigInteger=MagicMock(), Boolean=MagicMock(), DateTime=MagicMock(),
     ForeignKey=MagicMock(), Index=MagicMock(), Integer=MagicMock(),
     String=MagicMock(), Text=MagicMock(), UniqueConstraint=MagicMock())
_mod("sqlalchemy.ext")
_mod("sqlalchemy.ext.asyncio",
     AsyncSession=MagicMock, create_async_engine=MagicMock)
_mod("sqlalchemy.orm",
     sessionmaker=MagicMock, DeclarativeBase=MagicMock,
     Mapped=MagicMock, mapped_column=MagicMock, relationship=MagicMock)
...
_mod("utils.api",
     ...
     GameClient=MagicMock, AIEngineClient=MagicMock,
     SubscriptionClient=MagicMock, NotifierClient=MagicMock,
...
_mod("db.models",
     User=_UserModel, QuizGroup=_QuizGroupModel,
     QuizGroupSubscriber=MagicMock, Referral=_ReferralModel,
